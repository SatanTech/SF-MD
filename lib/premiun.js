...
const premium = JSON.parse(fs.readFileSync('./src/data/premium.json'))
/**
 * Add premium user.
 * @param {String} userId
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./src/data/premium.json", JSON.stringify(_dir));
};
 ...
 * @param {Object} _dir
 */
const expiredCheck = (arxzy, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./src/data/premium.json", JSON.stringify(_dir));
			idny ? arxzy.sendMessage(idny, { text: "Premium anda sudah habis silahkan untuk membeli lagi." }) : "";
			idny = false;
		}
	}, 1000);
};
